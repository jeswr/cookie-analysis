...
async function descriptionToPurposes(description: string = "Download certain Google Tools and save certain preferences, for example the number of search results per page or activation of the SafeSearch Filter. Adjusts the ads that appear in Google Search.") {
    ...
    const page = (await bindings.map(elem => (record[i++] = elem.get('concept')!.value) + '\t' + elem.get('definition')!.value + '\t' + elem.get('label')!.value + '\t' + elem.get('note')?.value).toArray()).join('\n');
    ...
    const chain = RunnableSequence.from([
        ...
        StructuredOutputParser.fromZodSchema(
            z.object({
                ...
                results: z.array(z.number().refine(num => Object.keys(record).includes(num.toString()), `Value must be one of ${JSON.stringify(Object.keys(record))}`)),
